...
router.get('/hourData/:id', function (req, res) {
	...
	function executeSelectStatement() 
	{
		try{
			...
			request = new Request("Select * from Room_THG_Info where s_infoId IN(select sensorId from SensorDetails where rId='"+RoomId+"') and THG_TS > DATEADD(hour, -1, GETUTCDATE()) ", function(err, rowCount, rows) {
			if (err)
			{ 
				error=true;
			}
	...
	{	
		try
		{
			...
			try
			{
				...
				var filePath ='Data.xlsx';		//specify file name
				...
				request.on('row', function(columns) {
					...
					try
					{
						...
						console.log('Row added successfully in sheet');
					}
					catch(err)
					{
						console.log(err);
		...
		{	
			...
			/*connection.close();
			reconnect();	*/		//send error to front end
		}
	}
	Read();
})
	...
	{	
		try
		{
			...
			try
			{
				...
				var filePath ='Data.csv';		//specify file name
				console.log("Filepath: "+filePath);
				sheet.columns = [					//add headers
					{ header: 'THG_InfoId', key: 'THG_InfoId' },
					{ header: 'THG_TS', key: 'THG_TS' },
